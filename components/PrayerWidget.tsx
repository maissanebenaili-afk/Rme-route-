...
export default function PrayerWidget() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch('/api/prayer?latitude=48.8566&longitude=2.3522&method=3', {
  ...
  return (
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-lg font-bold">🕌 Repère prières sur route</h2>
        <span className="text-xs text-slate-500">Position de référence actuelle : Paris</span>
      </div>
      <div className="mt-4 grid grid-cols-5 gap-2 text-center">
        {items.map(([name, time]) => (
          <div key={name} className="rounded-xl bg-slate-50 p-2">
            <div className="text-xs text-slate-500">{name}</div>
            <div className="mt-1 font-black">{time}</div>
          </div>
        ))}
      </div>
    </section>
